...
## 下一步：Provider 配置边界与真实 Provider 验收

1. 为 Provider Hub 中的 `future-*` 占位项增加不可配置说明，并为每一种可接入任务显示协议/路径要求，减少把语言模型误当多模态模型的配置错误。
2. 将资源库中的“恢复到创作页”扩展为可选的“恢复并再次生成”确认流程，始终由用户明确点击后才发起可能计费的请求。
