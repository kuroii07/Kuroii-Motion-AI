...
## 下一步：资产库跨页面复用与真实 Provider 验收

1. 在图片、音乐、配音和视频生成页加入“在资源库中查看”和“从资源库恢复参数/脚本”的回跳，避免用户在历史记录和资源库之间重复查找。
2. 为 Provider Hub 中的 `future-*` 占位项增加不可配置说明，并为每一种可接入任务显示协议/路径要求，减少把语言模型误当多模态模型的配置错误。
3. 使用用户自己的 MiniMax Key 做真实产物验收：图片、音乐、配音、视频各保留一个本地资产和不含密钥的诊断证据；不自动发起任何计费生成请求。
