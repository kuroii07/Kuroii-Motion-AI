...
function assetLibraryMediaUrl(asset, thumbnail = false) {
  if (!asset || !asset.assetType || !asset.id) return "";
  const query = new URLSearchParams({ session: "dev-local-token" });
  if (thumbnail) query.set("thumbnail", "1");
  return `http://127.0.0.1:17631/ai/assets/${encodeURIComponent(asset.assetType)}/${encodeURIComponent(asset.id)}/media?${query.toString()}`;
}
...
function loadAssetLibraryMedia(asset) {
  ...
  window.requestAnimationFrame(() => {
    ...
    const url = assetLibraryMediaUrl(asset);
...
async function loadAssetLibraryDetail(asset, options = {}) {
  ...
  state.assetLibraryDetail = null;
  state.assetLibraryDrawerOpen = true;
  clearAssetLibraryMedia();
  state.assetLibraryRegenerationConfirm = null;
  if (options.renderAfter !== false) render();
  try {
...
function musicDirectionOutputHtml(music) {
  ...
  if (music.status !== "ready") return `<div class="musicDirectionEmpty"><span class="statusDot muted"></span><strong>${state.locale === "zh-CN" ? "音乐预览" : "Music preview"}</strong><p>${state.locale === "zh-CN" ? "输入声音意图并生成音乐方向后，在这里查看可复用的提示词、创作蓝图和真实音频。" : "Enter a sound intent and build a music direction to review the reusable prompt, creative blueprint, and real audio here."}</p></div>`;
...
function bindMusicDirectionWorkspace() {
  const music = state.musicDirection;
  const musicReadiness = document.querySelector(".musicDirectionReadiness > .creationInspectorStatus");
  if (musicReadiness) musicReadiness.innerHTML = `<span class="statusDot ${capabilityConnectionTone("music")}"></span><div><span>${state.locale === "zh-CN" ? "音乐模型" : "Music model"}</span><strong>${escapeHtml(capabilityConnectionDetail("music", state.locale === "zh-CN" ? "当前仍可完成音乐方向设计" : "Direction design remains available."))}</strong></div>`;
...
function voiceDirectionOutputHtml(voice) {
  ...
  if (voice.status !== "ready") return `<div class="voiceDirectionEmpty"><span class="statusDot muted"></span><strong>${state.locale === "zh-CN" ? "配音预览" : "Voice preview"}</strong><p>${state.locale === "zh-CN" ? "输入脚本并拆分配音清单后，在这里查看分段、试听和真实音频。" : "Enter a script and build a voice list to review segments, listening previews, and real audio here."}</p></div>`;
...
function bindVoiceDirectionWorkspace() {
  const voice = state.voiceDirection;
  const voiceReadiness = document.querySelector(".voiceDirectionDelivery > .creationInspectorStatus");
  if (voiceReadiness) voiceReadiness.innerHTML = `<span class="statusDot ${capabilityConnectionTone("voice")}"></span><div><span>${state.locale === "zh-CN" ? "配音模型" : "Voice model"}</span><strong>${escapeHtml(capabilityConnectionDetail("voice", state.locale === "zh-CN" ? "本页不会生成假音频" : "This page never fabricates audio"))}</strong></div>`;
...
function renderMusicDirectionWorkbench(feature, workspace) {
  const music = state.musicDirection;
  workspace.setAttribute("aria-label", localText(feature.title));
  workspace.innerHTML = `<section class="taskWorkbench musicDirectionWorkbench">${workbenchHeaderHtml(feature, createModeToggleHtml())}<div class="musicDirectionDesk">
    <aside class="musicDirectionBriefPanel"><div class="railHeading"><strong>${state.locale === "zh-CN" ? "声音意图" : "Sound intent"}</strong><span>${state.locale === "zh-CN" ? "说明画面、受众、情绪与要强化的瞬间" : "Describe the visual, audience, emotion, and moments to amplify"}</span></div>
      <textarea id="musicDirectionBrief" placeholder="${state.locale === "zh-CN" ? "例如：奇幻 RPG 猫咪冒险的开场，轻快探索感，镜头从村庄推进到远方城堡。" : "Example: An opening for a fantasy RPG cat adventure, playful exploration, moving from village to distant castle."}">${escapeHtml(music.brief)}</textarea>
      <div class="musicDirectionControls"><label><span>${state.locale === "zh-CN" ? "使用场景" : "Use case"}</span><select id="musicDirectionUseCase"><option value="video" ${music.useCase === "video" ? "selected" : ""}>${state.locale === "zh-CN" ? "视频 / UA 配乐" : "Video / UA"}</option><option value="music" ${music.useCase === "music" ? "selected" : ""}>${state.locale === "zh-CN" ? "音乐先行探索" : "Music-first exploration"}</option></select></label><label><span>${state.locale === "zh-CN" ? "作品模式" : "Mode"}</span><select id="musicDirectionMode"><option value="instrumental" ${music.mode === "instrumental" ? "selected" : ""}>${state.locale === "zh-CN" ? "纯音乐" : "Instrumental"}</option><option value="song" ${music.mode === "song" ? "selected" : ""}>${state.locale === "zh-CN" ? "歌曲" : "Song"}</option></select></label></div>
      <p class="musicDirectionHint">${state.locale === "zh-CN" ? "未明确要求时，不在提示词中擅自限制时长或按秒划分结构。" : "No duration or second-by-second structure is inferred unless explicitly requested."}</p>
      <div class="musicDirectionAction"><button class="featurePrimaryButton" id="buildMusicDirectionButton" type="button">${state.locale === "zh-CN" ? "生成音乐方向" : "Build music direction"}</button><button class="featureSecondaryButton" id="generateMusicAudioButton" type="button" ${music.status === "generating" ? "disabled" : ""}>${music.status === "generating" ? (state.locale === "zh-CN" ? "生成中…" : "Generating…") : (state.locale === "zh-CN" ? "生成音乐" : "Generate music")}</button></div>
    </aside>
    <main class="musicDirectionStage" aria-live="polite">${musicDirectionOutputHtml(music)}</main>
    <aside class="musicDirectionReadiness"><section class="creationInspectorStatus" aria-label="${state.locale === "zh-CN" ? "音乐模型状态" : "Music model status"}"><span class="statusDot muted"></span><div><span>${state.locale === "zh-CN" ? "音乐模型" : "Music model"}</span><strong>${state.locale === "zh-CN" ? "正在读取模型状态" : "Reading model status"}</strong></div></section><section class="audioPlanningHistory"><div class="creationInspectorSectionHeader"><strong>${state.locale === "zh-CN" ? "本地资产记录" : "Local asset history"}</strong><button class="imageHistoryRefresh" id="refreshAudioHistoryButton" type="button">${state.locale === "zh-CN" ? "刷新" : "Refresh"}</button></div>${audioHistoryHtml("music-direction")}</section><div><p class="musicDirectionEyebrow">${state.locale === "zh-CN" ? "下一步" : "Next"}</p><ol><li>${state.locale === "zh-CN" ? "在 Provider 中登记支持音乐的模型" : "Register a music-capable model in Provider"}</li><li>${state.locale === "zh-CN" ? "绑定后再开放生成与试听" : "Enable generation and listening after binding"}</li></ol></div></aside>
  </div></section>`;
    ...
    <section class="taskWorkbench imageGenerationWorkbench videoGenerationWorkbench">
      ...
      <div class="imageGenerationWorkbenchBody">
        ...
        <aside class="imageGenerationInspector">
          <section class="creationInspectorStatus" aria-label="${state.locale === "zh-CN" ? "视频模型状态" : "Video model status"}">
            <span class="statusDot ${readiness.ready ? "success" : (readiness.status === "loading" ? "warning" : "muted")}"></span>
            <div><span>${state.locale === "zh-CN" ? "视频模型" : "Video model"}</span><strong>${escapeHtml(capabilityConnectionDetail("video", readyLabel))}</strong></div>
          </section>
          <section class="imageInspectorPanel imageHistorySection"><div class="creationInspectorSectionHeader"><strong>${state.locale === "zh-CN" ? "视频任务历史" : "Video task history"}</strong><button class="imageHistoryRefresh" id="refreshVideoTasksButton" type="button">${state.locale === "zh-CN" ? "刷新" : "Refresh"}</button></div>${videoTaskHistoryHtml()}</section>
        </aside>
      </div>
    </section>`;
...
function workbenchHeaderHtml(feature, meta = "") {
  ...
  // Creation modes keep their complete provider/model identity in the right
  // inspector.  The header is reserved for the shared mode switch and tools.
  const activeCreateCapability = state.activeView === "create" && ["text", "image", "video", "music", "voice"].includes(state.createMode)
    ? state.createMode
    : "";
    ...
    <section class="taskWorkbench documentWorkbench ${isCopy ? "copyDocumentWorkbench" : "contentDocumentWorkbench"}">
      ...
      <div class="documentWorkbenchBody">
        ...
        <main class="documentCanvas">
          ${isCreation ? "" : `<div class="documentRuler"><span>${isCopy ? (state.locale === "zh-CN" ? "候选版本" : "Variants") : (state.locale === "zh-CN" ? "生成文档" : "Generated document")}</span><div><button type="button">A</button><button type="button">B</button><button type="button">C</button></div></div>`}
          <article class="documentPage" aria-live="polite">
            ${textGenerationOutputHtml(feature, generation)}
          </article>
        </main>
        <aside class="documentOutlineRail">
          ${isCreation ? `<section class="creationInspectorStatus" aria-label="${state.locale === "zh-CN" ? "文案模型状态" : "Text model status"}"><span class="statusDot ${capabilityConnectionTone("text")}"></span><div><span>${state.locale === "zh-CN" ? "文案模型" : "Text model"}</span><strong>${escapeHtml(capabilityConnectionDetail("text"))}</strong></div></section>` : ""}
...
function renderLibraryWorkbench(feature, workspace) {
  ...
  const selected = items.find((item) => item.id === state.selectedAssetLibraryId) || null;
  const drawerOpen = Boolean(state.assetLibraryDrawerOpen && selected);
